...
class Key:
  ...
  def __generate_round_keys(self):
    ...
    for i in range(0, 16):
      ...
      round_key = Operation.permute(
          left_key + right_key, self.SECOND_COMPRESSION_PERMUTATION)
...
class Round:
  ...
  def __process(self):
    for i in range(16):
      operated_right = Operation.permute(
          self.right_plain_text, self.EXPANSION_PERMUTATION)
      ...
      self.left_plain_text = Operation.xor(
          self.left_plain_text, operated_right)
  ...
  def __substitution(self, operated_right):
    ...
    for i in range(8):
      index = i * 6
      row = Binary.to_dec(
          operated_right[index] + operated_right[index + 5])
...
class Data:
  def __init__(self, plain_text):
    ...
    self.data = plain_text.ljust(
        math.ceil(len(plain_text) / self.block_size) * self.block_size, "\0")
...
class DES:
  ...
  def __process(self, round_keys):
    ...
    for datum in self.data.get_list():
      ...
      operated_text = Round(
          plain_text[:32], plain_text[32:], round_keys).get_operated_text()
      result = result + \
          Operation.permute(operated_text, self.FINAL_PERMUTATION)
